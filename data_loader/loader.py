...
class IAMDataset(Dataset):
    ...
    def collate_fn_(self, batch):
        ...
        # Ensure width is divisible by 8 for U-Net compatibility
        max_width = max(width)
        padded_width = ((max_width + 7) // 8) * 8  # Round up to nearest multiple of 8
        
        # Also ensure style width is divisible by 8
        max_s_width = ((max_s_width + 7) // 8) * 8

        imgs = torch.ones([len(batch), batch[0]['img'].shape[0], batch[0]['img'].shape[1], padded_width], dtype=torch.float32)
        ...
        for idx, item in enumerate(batch):
            ...
            try:
                # Determine how much style width to use
                style_width_to_use = min(item['style'].shape[2], self.style_len)
                style_ref[idx, :, :, 0:style_width_to_use] = item['style'][:, :, :style_width_to_use]
                laplace_ref[idx, :, :, 0:style_width_to_use] = item['laplace'][:, :, :style_width_to_use]
            except:
