...
class Trainer:
    ...
    def _train_iter(self, data, step, pbar):
        ...
        if True:  # Always log in single GPU mode
            # log file
    ...
    def _valid_iter(self, epoch):
        ...
        for text in texts:
            rank = 0  # Single GPU mode, always rank 0
    ...
    def train(self):
        ...
        for epoch in range(cfg.SOLVER.EPOCHS):
            # Only call set_epoch if sampler has this method (DistributedSampler)
    ...
    def _save_checkpoint(self, epoch):
        # Handle both DDP and single GPU models
        model_state = self.model.module.state_dict() if hasattr(self.model, 'module') else self.model.state_dict()
        torch.save(model_state, os.path.join(self.save_model_dir, str(epoch)+'-'+"ckpt.pt"))