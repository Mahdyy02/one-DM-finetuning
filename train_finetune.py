...
def main(opt):
    ...
    if len(opt.one_dm) > 0:
        unet.load_state_dict(torch.load(opt.one_dm, map_location=torch.device('cpu'), weights_only=False))
    ...
    if len(opt.ocr_model) > 0:
        miss, unxep = ocr_model.load_state_dict(torch.load(opt.ocr_model, map_location=torch.device('cpu'), weights_only=False), strict=False)
